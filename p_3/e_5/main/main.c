...
static void IRAM_ATTR gpio_input_isr_handler(void *args)
{
    // tag
    static const char *TAG = "GPIO_INPUT_ISR_HANDLER";
...
void app_main(void)
{
    ...
    // set GPIO input as interrupt
    ESP_ERROR_CHECK(
        gpio_set_intr_type(GPIO_INPUT, GPIO_INTR_ANYEDGE)
    );
    ...
    // set GPIO input interrupt handler
    ESP_ERROR_CHECK(
        gpio_isr_handler_add(GPIO_INPUT, gpio_input_isr_handler, NULL)
    );
    ESP_LOGI(TAG, "GPIO input interrupt handler set");
}
